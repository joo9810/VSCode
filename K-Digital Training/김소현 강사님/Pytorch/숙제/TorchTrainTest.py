    ...
    with torch.no_grad(): # 가중치 업데이트 없이 테스트 진행
        for X_batch, y_batch in test_DataLoader:
            ...
            if is_reg == True:
                ...
                y_batch = y_batch.reshape(-1) # 다중 분류는 y가 반드시 1차원이어야 함.. (너무 불친절)
                loss_test = F.cross_entropy(pred_test_y, y_batch.long())
    ...
    for i in range(1, epoch+1): # 에포크 횟수만큼 반복
        ...
        for X_batch, y_batch in train_DataLoader:
            ...
            if is_reg == True:
                ...
                y_batch = y_batch.reshape(-1) # 다중 분류는 y가 반드시 1차원이어야 함.. (너무 불친절)
                loss_train = F.cross_entropy(pred_train_y, y_batch.long())
                # 다중 분류는 long타입으로 전달해야 하는듯
        ...
        if is_reg == True:
            loss_test_avg = testing(test_DataLoader, model, is_reg=True)
        elif is_reg == False and is_bin == True:
            loss_test_avg = testing(test_DataLoader, model, is_reg=False, is_bin=True)
        elif is_reg == False and is_bin == False:
            loss_test_avg = testing(test_DataLoader, model, is_reg=False, is_bin=False)
        ...
        # (4) 학습 결과 출력
        if i % 10 == 0:
            print(f"[Train : {i}/{epoch}] Loss : {loss_train_avg}")
            print(f"[Test  : {i}/{epoch}] Loss : {loss_test_avg}")
